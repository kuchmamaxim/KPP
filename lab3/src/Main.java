        ...
        calculateButton.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent e) {
                for (int k = 0; k < Double.parseDouble(nTextField.getText()); k++) {
                    ...
                    ComplexThread calculater = new ComplexThread(Double.parseDouble(xTextField.getText()), Double.parseDouble(yTextField.getText()), Integer.parseInt(nTextField.getText()), k, exchanger);
                    calculater.start();
                    String result = null;
                    try {
                        result = exchanger.exchange(new String("mes"));
                    } catch (Exception ex) {
                    }
